...
### 一键分发
- 每条帖子操作栏末尾自带分发图标（与原生按钮同宽度的弹性格子，间距对齐），点击弹出悬浮窗
...
- 发布完成后，进度小窗里可**一键关闭本次分发的全部标签页**（整个分组）；小窗内显示目标平台名称与已打开的发布页列表
- 即刻视频分发：自动定位 composer 的视频文件输入（支持懒挂载场景，找不到时主动点击媒体入口再等待）
...
- **预设**：把整套样式保存为命名预设，随时一键应用；下拉第一项「默认预设」可随时回到默认样式；水印支持平铺文字（文字 / 颜色 / 密度 / 角度 / 透明度）
- 名称过长自动缩小字号避免裁切；深浅色主题自动跟随 x.com
- 悬浮窗底部有作者信息：打赏（赞赏码）与作者主页入口（小红书 / 抖音 / X）
